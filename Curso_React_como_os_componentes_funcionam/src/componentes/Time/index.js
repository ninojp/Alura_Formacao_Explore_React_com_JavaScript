...
const Time = ({time, colaboradores, aoDeletar, mudarCor2, mudarCorF}) => {
    return (
        (colaboradores.length > 0) && <section className='time' style={{ backgroundImage: 'url(/imagens/fundo.png)', backgroundColor: time.corPrimaria }}>
            ...
            <input onChange={evento => mudarCor2(evento.target.value, time.nome)} value={time.corSecundaria} type='color' className='input-cor' />
            <span className='span-inputF'>Cor do Fundo</span>
            <input onChange={event => mudarCorF(event.target.value, time.nome)} value={time.corPrimaria} type='color' className='input-corF' />
